...
const AchievementPopup = () => {
    ...
    useEffect(() => {
        ...
        const timer = setTimeout(dismissActiveSecret, 4500);
        return () => clearTimeout(timer);
    }, [activeSecret, dismissActiveSecret]);

    if (activeSecret) {
        return (
            <div className="achievement-popup completed interactive" style={{ pointerEvents: 'auto' }} onClick={dismissActiveSecret}>
                <svg
                ...
                <div className="popup-content">
                    ...
                    <div className="text-content">
                        <span className="title">{activeSecret.title}</span>
                        <span className="description">{activeSecret.line}</span>
                        {isBlueprintUnlocked && (
                            <span className="description">Blueprint Mode and The Draft Room are now available.</span>
                        )}
                    </div>
                </div>
            </div>
