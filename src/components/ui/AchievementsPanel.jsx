...
const AchievementsPanel = ({ isOpen, onClose }) => {
    ...
    const { secretCount, secretTotal, isBlueprintUnlocked, openDraftRoom } = useSecrets();
        ...
        <div className={`achievements-panel ${isOpen ? 'open' : ''}`} inert={!isOpen ? true : undefined}>
            <div className="achievements-card">
                ...
                <div className="achievements-footer">
                    ...
                    <span>{isBlueprintUnlocked ? 'DRAFT ROOM AVAILABLE' : 'KEEP EXPLORING'}</span>
                    {isBlueprintUnlocked && (
                        <button type="button" className="draft-room-link" onClick={openDraftRoom}>
                            Open The Draft Room
                        </button>
                    )}
                </div>
