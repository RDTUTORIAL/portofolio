...
const ContentCard = ({ content, isOpen, onClose, isMobile }) => {
    ...
    const hasExternalUrl = rawUrl && rawUrl !== '#' && rawUrl !== '-' && rawUrl.toLowerCase() !== 'coming soon';
    const disabledActionLabel = content.actionLabel || 'PRIVATE';

    const [showGallery, setShowGallery] = useState(false);

    // Reset gallery state when overlay closes
    useEffect(() => {
        if (!isOpen) {
    ...
    const cardStyle = isMobile ? {
        ...
        opacity: isOpen ? 1 : 0,
        color: '#1a1a1a',
        zIndex: 2, // main card above gallery
    };

    // Gallery Card Style (sliding out from behind the main card)
    const galleryCardStyle = isMobile ? {
        // MOBILE: Slide down from main card
        width: '90%',
        height: '40vh',
        bottom: 'calc(10rem + 65vh)',
        left: '50%',
        transform: showGallery ? 'translate(-50%, 0)' : 'translate(-50%, 50%)',
        opacity: showGallery ? 1 : 0,
        pointerEvents: showGallery ? 'auto' : 'none',
        zIndex: 1,
    } : {
        // DESKTOP: Slide left from main card
        width: 'clamp(300px, 40vw, 600px)',
        height: 'clamp(400px, 70vh, 800px)',
        right: 'clamp(2rem, 12vw, 20rem)', // anchor to same point as main card
        top: '50%',
        // Slide out to the left by translating -100% minus a gap
        transform: showGallery ? 'translate(calc(-100% - 2rem), -50%) rotate(-1deg)' : 'translate(0, -50%) rotate(1deg)',
        opacity: showGallery ? 1 : 0,
        pointerEvents: showGallery ? 'auto' : 'none',
        zIndex: 1,
    };
    ...
    return (
        ...
        >
            ...
            >
                            ...
                            <div style={{
                                ...
                                {content.gallery ? (
                                    <button
                                        ...
                                        style={{ border: 'none', cursor: 'pointer', fontFamily: "'Cabin Sketch', cursive", fontSize: '1.2rem' }}
                                    >
                                        {showGallery ? 'Hide Documentation ←' : 'View Documentation ↗'}
                                    </button>
                                ) : hasExternalUrl ? (
                                    <a
                                        href={rawUrl}
                                    ...
                                    <button
                                        type="button"
                                        disabled
                                        className="studio-action-button is-disabled"
                                    >
                                        {disabledActionLabel}
                                    </button>
                                )}
                ...
                {/* === SECONDARY PANEL: GALLERY === */}
                {content.layout === 'story' && content.gallery && (
                    <div
                        style={{
                            position: 'absolute',
                    ...
                    >
                        <div style={{
                            ...
                            <h3 style={{ margin: 0, fontSize: '1.5rem', fontWeight: 800, fontFamily: "'Rubik Scribble', cursive" }}>
                                Documentation
                            </h3>
                            <button
