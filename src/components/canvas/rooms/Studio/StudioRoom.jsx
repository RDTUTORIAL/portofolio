...
const HiddenTerminalSecret = ({ onClick }) => (
    <group position={[2.05, -0.55, -7.6]} rotation={[0, -0.32, 0]}>
        <mesh
        ...
        >
            ...
            <meshBasicMaterial color={isBlueprintMode() ? BLUEPRINT_THEME.background : '#161616'} wireframe={isBlueprintMode()} />
        </mesh>
        <Text
            position={[0, 0, 0.06]}
            fontSize={0.105}
            color={isBlueprintMode() ? BLUEPRINT_THEME.line : '#8ee6a8'}
            anchorX="center"
            anchorY="middle"
        ...
        <mesh position={[0, 0, 0.12]} onPointerDown={onClick}>
...
const MIN_LOOP_RINGS = 6; // Repeat content enough times to fill the full vertical loop
...
const StudioRoom = ({ showRoom, onReady, isExiting, isWarmup }) => {
    ...
    const monitorData = useMemo(() => {
        ...
        // Repeat the project list so the vertical stream is always full, even
        // when Studio has fewer unique projects than the old placeholder data.
        const totalMonitors = Math.max(
            shuffledContent.length,
            MONITORS_PER_RING * MIN_LOOP_RINGS
        );
        ...
        for (let ring = 0; ring < ringsNeeded && contentIndex < totalMonitors; ring++) {
            const angleStep = (Math.PI * 2) / MONITORS_PER_RING;
            const angleOffset = ring % 2 === 0 ? 0 : angleStep / 2; // Offset alternate rings

            for (let i = 0; i < MONITORS_PER_RING && contentIndex < totalMonitors; i++) {
                const sourceItem = shuffledContent[contentIndex % shuffledContent.length];
                const contentItem = {
                    ...sourceItem,
                    id: `${sourceItem.id}-loop-${contentIndex}`
                };
    ...
    return (
        <group ref={groupRef} position={[0, -1.2, 0]}>
            ...
            <HiddenTerminalSecret onClick={handleHiddenTerminalClick} />
...
const MonitorBlock = memo(({ item, meshRef, isSelected, onMonitorClick, disabled, paintOnBeforeCompile, paintUniforms }) => {
    ...
    const isBlogMonitor = item.platform === 'web';
    const isTvMonitor = item.platform === 'ai_cyber';
    const isPhoneMonitor = item.platform === 'iot';
        ...
        >
            ...
            <mesh>
                ...
                {faceConfig.map((face, i) => {
                    if (face.painted && i === FRONT_FACE_INDEX) {
                        // Keep the expensive brush reveal on the screen/front only.
                        // Side and back faces stay as regular sketch materials for a lighter infinite tower.
                        return (
