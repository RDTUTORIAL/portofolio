        ...
        >
            ...
            {!isTouch && (
                <mesh ref={framePaintedRef} position={[0, 0, -0.001]} scale={[0.98, 0.98, 1]}>
                    <planeGeometry args={[frame.width, frame.height]} />
                    <meshBasicMaterial color="#e0e0e0"
                        map={frame.customFramePaintedTexture || framePaintedTexture}
                        transparent={true}
                        alphaTest={0.5}
                        side={THREE.DoubleSide}
                        roughness={0.9}
            ...
            <mesh position={[0, 0, 0]}>
                ...
                <revealMaterial color="#e0e0e0"
                    ...
                    map={frame.customFrameTexture || frameTexture}
                    transparent={true}
                    alphaTest={0.1}
                    side={THREE.DoubleSide}
                    roughness={0.9}
            ...
            {frame.titleLabel && (
                <Text
                    position={[
                        frame.titleX !== undefined ? frame.titleX : 0,
                        frame.titleY !== undefined ? frame.titleY : (frame.height / 2 + 0.2),
                        0.03
                    ]}
                    fontSize={frame.titleSize || 0.16}
                    font="/fonts/CabinSketch-Bold.ttf"
                    color={frame.titleColor || "#222222"}
                    anchorX="center"
                    anchorY="middle"
                >
                    {frame.titleLabel}
...
const TableCoffeeSecret = ({ texture, steamTexture, position }) => {
    const { discoverSecret } = useSecrets();
    const groupRef = useRef();
    ...
    useFrame((state) => {
        ...
        const wave = 0.35 + Math.sin(state.clock.elapsedTime * 3) * 0.12;
        steamRef.current.material.opacity = isActive ? wave : 0.16 + Math.sin(state.clock.elapsedTime * 2) * 0.04;
    });

    const revealCoffee = (e) => {
        e.stopPropagation();
        setIsActive(true);
    ...
    return (
        <group ref={groupRef} position={position} rotation={[0, Math.PI / 2, 0]}>
            ...
            <mesh ref={steamRef} position={[0.02, 0.49, 0.01]} raycast={() => null}>
                <planeGeometry args={[0.5, 0.34]} />
                <meshBasicMaterial
                    color={isActive ? '#ffffff' : '#f1eee7'}
                    map={steamTexture}
                    transparent
                    opacity={0.18}
                    alphaTest={0.04}
...
const Corridor404Secret = ({ position, rotation }) => {
    const { discoverSecret } = useSecrets();
    const [isFound, setIsFound] = useState(false);
    
    // Load textures inside component
    const texture404 = useTexture('/textures/corridor/404_frame.webp');
    const textureFound = useTexture('/textures/corridor/found_frame.webp');
    ...
    return (
        ...
        >
            ...
            <meshBasicMaterial color="#e0e0e0"
                map={isFound ? textureFound : texture404}
                transparent={true}
                alphaTest={0.1}
                side={THREE.DoubleSide}
                roughness={0.8}
...
const CorridorDecorations = ({ segmentLength, zOffset, corridorWidth = 4, corridorHeight = 3.5, zClip = 100000, setCameraOverride }) => {
    ...
    const privateDraftsFrameTexture = useTexture('/textures/corridor/private_drafts_frame.webp');
    const privateDraftsFramePaintedTexture = useTexture('/textures/corridor/private_drafts_frame_painted.webp');
    ...
    useMemo(() => {
        ...
        coffeeSteamTexture.colorSpace = THREE.SRGBColorSpace;
    }, [coffeeTexture, coffeeSteamTexture]);
    ...
    const frames = useMemo(() => [
        ...
        },
        {
            ...
            height: 2.5 / 1.785,
            y: 0.25,
            id: 'frame-private-drafts',
            offsetFromWall: 0.1,
            secretId: SECRET_IDS.CORRIDOR_FRAME,
            customFrameTexture: privateDraftsFrameTexture,
            customFramePaintedTexture: privateDraftsFramePaintedTexture
        },
        {
        ...
        <group>
            ...
            <group position={[tableConfig.x, floorY, tableConfig.z]} rotation={[0, Math.PI / 2, 0]}>
                ...
                <TableCoffeeSecret
                    ...
                    steamTexture={coffeeSteamTexture}
                    position={[-0.48, tableConfig.height + tableConfig.topThickness + 0.23, 0.18]}
                />
            </group>
            ...
            */}
            ...
            {/* === STOJĄCA RAMKA NA SZAFCE (STANDING FRAME) TERAZ JAKO EASTER EGG 404 === */}
            <Corridor404Secret
