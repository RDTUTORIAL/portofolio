 ...
 * 3. Behind doors: infinite corridor with Danis
 */
const Experience = ({ isLoaded, onSceneReady, performanceTier }) => {
    ...
    // Disable camera scroll when any overlay that steals scroll is open
    const { isDraftRoomOpen } = useSecrets();
    ...
    const { setCameraOverride } = useInfiniteCamera({
        ...
        smoothing: 0.06,
        scrollEnabled: hasEntered && !isTeleporting && !isInRoom && !isDraftRoomOpen,
        parallaxEnabled: hasEntered && !isTeleporting && !isInRoom && !isDraftRoomOpen
    });
